...
int main(void)
{
        ...
        server_addr.sin_port = htons(2012);               // Set port number, using htons function to use proper byte order */
     ...
     // The listen() function places all incoming connection into a "backlog queue" until accept() call accepts the connection.
        ...
        while(1){
                ...
                char res[2000] = "Hello I am server. Your received id is ";
                res[strlen(res)] = client_message[strlen(client_message) - 1];
                res[strlen(res)] = '\0';
